...
const DATA_TYPES = {
  courses: { 
    ...
    example: "CourseCode,CourseName,Instructor,Semester,Credits,Monday,Tuesday,Wednesday,Thursday,Friday"
  },
  assignments: { 
  ...
  detailed_grades: { 
    ...
    icon: GraduationCap, 
    description: "Import comprehensive grade data with individual assignment scores",
    example: "Semester,CourseCode,CourseName,Instructor,Credits,Assignment1,Quiz1,Midterm,Final,FinalGrade,GPA"
  },
  transcript: { 
    label: "Official Transcripts", 
    icon: BarChart3, 
    description: "Import official transcript data with historical grades",
    example: "Year,Semester,CourseCode,CourseName,Credits,FinalGrade,GradePoints,Percentage"
  },
  study_sessions: { 
    label: "Study Session Logs", 
    icon: Clock, 
    description: "Import study time tracking and productivity data",
    example: "Date,CourseCode,Title,Duration,SessionType,FocusScore,ProductivityRating,Location,Notes"
  },
  schedule: { 
    label: "Course Schedules", 
    icon: Calendar, 
    description: "Import detailed class schedules with times and locations",
    example: "CourseCode,CourseName,Instructor,Semester,Monday,Tuesday,Wednesday,Thursday,Friday,Location"
  }
};
...
export function DataImportExportHub() {
  ...
  // Queries
  const importJobs = useQuery(api.dataImportExport.getImportExportJobs, 
    user ? { userId: user.clerkId } : "skip"
  );
  
  const importStatus = useQuery(api.userContext.getImportExportStatus,
    user ? { userId: user.clerkId } : "skip"
  );

  // Mutations
  const createImportJob = useMutation(api.dataImportExport.createImportExportJob);
  ...
  const handleStartImport = async () => {
    if (!user || !uploadedFile || !selectedDataType) {
      sonnerToast.error('Missing Information — Please select a data type and upload a CSV file.');
      return;
    }
    ...
    try {
      ...
      sonnerToast.success(`Started importing ${selectedDataType} data from ${uploadedFile.name}`);
      ...
      sonnerToast.error('Import Failed — Failed to start the import process. Please try again.');
    }
  };
  ...
  const handleExportData = async (dataType: string) => {
    ...
    try {
      ...
      sonnerToast.success(`Started exporting ${dataType} data to CSV format`);

    } catch (error) {
      sonnerToast.error('Export Failed — Failed to start the export process. Please try again.');
    }
  };
      ...
      {importStatus && (
        <div className="grid gap-4 md:grid-cols-4">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{importStatus.importStats.totalImports}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent className="space-y-6">
              <div className="grid gap-4">
                <div>
                  ...
                  {selectedDataType && (
                    <div className="mt-2 text-sm text-muted-foreground">
                      {DATA_TYPES[selectedDataType as keyof typeof DATA_TYPES]?.description}
                    </div>
                  )}
                </div>
                ...
                {selectedDataType && (
                  <div className="p-4 bg-muted rounded-lg">
                    ...
                    <code className="text-xs bg-background p-2 rounded block overflow-x-auto">
                      {DATA_TYPES[selectedDataType as keyof typeof DATA_TYPES]?.example}
                    </code>
                  </div>
                )}
                ...
                <Button 
                  ...
                  disabled={!selectedDataType || !uploadedFile || !importStatus?.readyForImport}
                  className="w-full"
                >
                  {importStatus?.readyForImport ? "Start Import" : "Another import is in progress..."}
                </Button>
              </div>
            </CardContent>
          </Card>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h4 className="font-medium">
                            {job.operation === "import" ? "Import" : "Export"} {job.dataType}
                          </h4>
