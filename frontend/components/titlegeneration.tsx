...
function TitleGenerations({ videoId }: { videoId: string }) {
  const { user } = useUser();
  // Only call the query when both videoId and user.id are available to avoid
  // sending undefined to Convex (which causes ArgumentValidationError).
  const titles =
    videoId && user?.id
      ? useQuery(api.titles.list, { videoId, userId: user.id })
      : undefined;
